...
const MENU_SETTINGS_SHEET_NAME = "Menu settings";
  ...
  try {
    ...
    try {
      ...
      sheet.appendRow([
        ...
        "New",
        safeCell(order.name),
        whatsAppLink(order.phone),
        safeCell(order.bakeWindow),
        safeCell(order.items),
        safeCell(order.estimatedTotal),
        safeCell(order.delivery),
        safeCell(order.pickupTime),
