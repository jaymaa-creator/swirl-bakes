    ...
    >
      <div className="max-w-5xl">
        <div className="max-w-3xl">
          <div className="text-xs uppercase tracking-[0.2em] text-inkMuted">Menu</div>
          <h2 className="mt-2 text-3xl text-ink sm:text-4xl">
            {batchLabel ? `Bakes for ${batchLabel}` : "Next available bake"}
          </h2>
          <p className="mt-3 text-sm leading-7 text-inkMuted sm:text-[1rem]">
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-2">
          {visibleItems.map((m) => {
            ...
            const isFeatured = m.special === true && isAvailable;
            ...
            return (
              <div
                ...
                className={`overflow-hidden rounded-3xl border bg-surface shadow-[0_8px_24px_rgba(90,56,37,0.08)] ${
                  isFeatured ? "lg:col-span-2 lg:grid lg:grid-cols-2 border-[#E7C5A3]" : isAvailable ? "border-line" : "border-[#E7C5A3] bg-[#FFF9F2]"
                }`}
              >
                {isFeatured ? (
                  <div className="border-b border-[#E7C5A3] bg-[linear-gradient(120deg,#FFF7E9,#F4C98C)] px-5 py-4 text-xs font-bold uppercase tracking-[0.2em] text-brandBrown lg:col-span-2">
                    Weekly special
                  </div>
                ) : null}
                {m.image ? (
                      ...
                      <>
                        ...
                        <div className="pointer-events-none absolute right-3 top-3 w-1/4 min-w-20 max-w-32">
                          <img
                            ...
                            alt="Swirl Girl says sold out for this bake"
                          ...
                          <div className="relative -mt-2 ml-[-22%] rounded-2xl border border-[#E7C5A3] bg-[#FFF1DF] px-2 py-1.5 text-center text-[10px] font-semibold leading-tight text-brandBrown shadow-[0_3px_8px_rgba(90,56,37,0.12)] sm:text-xs">
                            ...
                            <span className="relative">Sold out for this bake</span>
                          </div>
                        </div>
                      </>
                    ) : null}
                ...
                <div className={`min-w-0 px-5 py-6 ${isFeatured ? "lg:self-center lg:px-8 lg:py-8" : ""}`}>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="text-2xl text-ink">{m.name}</div>
                    </div>
                    {isAvailable && !isFeatured && (m.special || m.badge) ? (
                  ...
                  <div className="mt-4 text-sm font-semibold text-ink">{money(m.priceSgd)} {m.unitLabel || "each"}</div>
                  ...
                  {m.allergens ? <div className="mt-4 text-xs text-inkMuted">{m.allergens}</div> : null}
                </div>
              </div>
            );
          })}
